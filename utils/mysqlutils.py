...
from utils import log_client


logger = log_client("index")
...
def connect_mysql(config_dict=None):
    ...
    try:
        config = {
            'host': 'rm-uf698x9pde1ytqxe8ko.mysql.rds.aliyuncs.com',
            'port': 3306,
            'user': 'opera_man_prod',
            'passwd': 'nengliang2019@',
            'charset': 'utf8mb4',
...
def updateSql(sql, param=None):
    ...
    con = connect_mysql()
    cur = con.cursor()
    row = update_process(con, cur, sql, param)
    cur.close()
    con.close()
    return row
...
def update_process(con, cur, sql, param=None):
    """" queryone:内部调用 """
    row = None
    try:
        cur.execute(sql, param)
...
def queryall_source(sql, param=None):
    """
    返回所有查询到的内容 (分页要在sql里写好)
    :param sql: sql语句
    :param param: tuple|list
